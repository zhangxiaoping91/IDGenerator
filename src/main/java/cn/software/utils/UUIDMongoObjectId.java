 ...
 * @Description ：根据monogdb的objectId生成UUID
 * @ClassName ：UUIDMongoObjectId
 * @date ：2017/4/6 15:15
 */
public class UUIDMongoObjectId implements Serializable, Comparable<UUIDMongoObjectId> {
...
//    static final Logger LOGGER = Loggers.getLogger("ObjectId");
     ...
     * @return the new id
     */
    public static UUIDMongoObjectId get() {
        return new UUIDMongoObjectId();
    }
     ...
     * @since 2.12.0
     */
    public static UUIDMongoObjectId createFromLegacyFormat(final int time, final int machine, final int inc) {
        return new UUIDMongoObjectId(time, machine, inc);
    }

    /**
     * Create a new object id.
     */
    public UUIDMongoObjectId() {
        this(new Date());
    }
     ...
     * @param date the date
     */
    public UUIDMongoObjectId(final Date date) {
     ...
     */
    public UUIDMongoObjectId(final Date date, final int counter) {
     ...
     */
    public UUIDMongoObjectId(final Date date, final int machineIdentifier, final short processIdentifier, final int counter) {
     ...
     * @throws IllegalArgumentException if the high order byte of machineIdentifier or counter is not zero
     */
    public UUIDMongoObjectId(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter) {
        this(timestamp, machineIdentifier, processIdentifier, counter, true);
    }

    private UUIDMongoObjectId(final int timestamp, final int machineIdentifier, final short processIdentifier, final int counter,
     ...
     */
    public UUIDMongoObjectId(final String hexString) {
     ...
     */
    public UUIDMongoObjectId(final byte[] bytes) {
     ...
     */
    UUIDMongoObjectId(final int timestamp, final int machineAndProcessIdentifier, final int counter) {
     ...
     * @since 3.4
     */
    public UUIDMongoObjectId(final ByteBuffer buffer) {
    ...
    public boolean equals(final Object o) {
        ...
        UUIDMongoObjectId objectId = (UUIDMongoObjectId) o;
    ...
    public int compareTo(final UUIDMongoObjectId other) {
        if (other == null) {
